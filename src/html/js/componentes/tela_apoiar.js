 ...
 * secao 11). Só exibe as strings "Pix copia e cola" já prontas
 * (`js/dados_apoio.js`) — nunca constrói/calcula payload aqui. Sem nenhuma
 * opção com `payload` preenchido, mostra um aviso de "em breve" em vez de
 * uma tela quebrada.
 */
export class TelaApoiar extends HTMLElement {
    ...
    connectedCallback() {
        ...
        this._montarPainelPix();
    }

    _montarPainelPix() {
        const valoresEl = this.querySelector(".pix-valores");
        const qrEl = this.querySelector(".pix-qr");
        const copiarBtnEl = this.querySelector(".pix-copiar");
        const copiaManualEl = this.querySelector(".pix-copia-manual");
        const indisponivelEl = this.querySelector(".pix-indisponivel");
